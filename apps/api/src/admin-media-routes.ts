...
const completeSchema = z.object({
  ...
  type: z.enum(["IMAGE", "VIDEO", "RENDER", "FLOOR_PLAN", "VIRTUAL_TOUR"]), placement: z.enum(["APARTMENT_GALLERY", "APARTMENT_PLAN", "INTERIOR", "VIRTUAL_TOUR"]), isPublished: bool.default(false), isCover: bool.default(false),
});
...
adminMediaRouter.post("/apartments/:slug/media/sign", requireAdmin, writers, asyncHandler(async (req, res) => {
  ...
  res.json({ ok: true, upload: { cloudName: env.cloudinary.cloudName, apiKey: env.cloudinary.apiKey, timestamp, signature, folder, resourceType: body.resourceType } });
}));

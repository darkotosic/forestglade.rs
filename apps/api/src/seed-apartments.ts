...
for (const apartment of OFFICIAL_APARTMENTS) {
  ...
  await prisma.apartment.upsert({
    where: { code },
    update: { slug, floor, officialType, marketArea, sourceLocked: true, isPublished: true, sortOrder },
    create: { code, slug, floor, officialType, marketArea, sourceLocked: true, status: "AVAILABLE", isPublished: true, sortOrder },
  });
}

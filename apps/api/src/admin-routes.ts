...
import { z } from "zod";
import { clearSessionCookie, hashToken, newSessionToken, sessionExpiresAt, setSessionCookie, newSessionToken as newCsrfToken } from "./auth.js";
...
adminRouter.get("/auth/csrf", (_req, res) => { const token = newCsrfToken(); res.cookie(csrfCookieName, token, { httpOnly: false, secure: env.nodeEnv === "production", sameSite: "lax", path: "/" }); res.json({ ok: true, token }); });
adminRouter.use((req, res, next) => { if (!["POST", "PATCH", "DELETE"].includes(req.method) || ["/auth/login", "/auth/bootstrap"].includes(req.path)) return next(); const cookieToken = req.cookies?.[csrfCookieName]; if (!cookieToken || req.header("x-csrf-token") !== cookieToken) return res.status(403).json({ ok: false, message: "CSRF token nije ispravan." }); next(); });
...
adminRouter.patch("/settings", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const changed=[]; for(const [key,value] of Object.entries(req.body)) changed.push(await prisma.siteSetting.upsert({where:{key},update:{value:value as never},create:{key,value:value as never}})); await logAudit({adminId:req.admin?.id,action:"UPDATE",entity:"SiteSetting",afterJson:req.body,req}); res.json({ok:true,settings:changed}); });

adminRouter.get("/users", requireAdmin, requireRole([...roles.manage]), async(_req,res)=>res.json({ok:true,users:await prisma.adminUser.findMany({orderBy:{createdAt:"desc"},select:{id:true,email:true,name:true,role:true,isActive:true,lastLoginAt:true,createdAt:true}})}));
adminRouter.post("/users", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const body=z.object({email:z.string().email(),name:z.string().min(2),password:z.string().min(10),role:z.enum(["OWNER","ADMIN","SALES","EDITOR"])}).parse(req.body); if(req.admin?.role!=="OWNER" && !["SALES","EDITOR"].includes(body.role)) throw new ApiError(403,"Nemate dozvolu za ovu rolu.","FORBIDDEN"); const user=await prisma.adminUser.create({data:{email:body.email.toLowerCase(),name:body.name,passwordHash:await bcrypt.hash(body.password,12),role:body.role}}); await logAudit({adminId:req.admin?.id,action:"CREATE",entity:"AdminUser",entityId:user.id,afterJson:user,req}); res.status(201).json({ok:true,user}); });
adminRouter.patch("/users/:id", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const before=await prisma.adminUser.findUniqueOrThrow({where:{id:String(req.params.id)}}); if(req.admin?.role!=="OWNER" && before.role==="OWNER") throw new ApiError(403,"OWNER ne može biti izmenjen.","FORBIDDEN"); const body=z.object({name:z.string().min(2).optional(),role:z.enum(["OWNER","ADMIN","SALES","EDITOR"]).optional(),isActive:z.boolean().optional()}).parse(req.body); if(req.admin?.role!=="OWNER" && body.role && !["SALES","EDITOR"].includes(body.role)) throw new ApiError(403,"Nemate dozvolu za ovu rolu.","FORBIDDEN"); const user=await prisma.adminUser.update({where:{id:String(req.params.id)},data:body}); await logAudit({adminId:req.admin?.id,action:"UPDATE",entity:"AdminUser",entityId:user.id,beforeJson:before,afterJson:user,req}); res.json({ok:true,user}); });
adminRouter.post("/users/:id/reset-password", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const body=z.object({password:z.string().min(10)}).parse(req.body); const user=await prisma.adminUser.update({where:{id:String(req.params.id)},data:{passwordHash:await bcrypt.hash(body.password,12)}}); await logAudit({adminId:req.admin?.id,action:"UPDATE",entity:"AdminUser",entityId:user.id,message:"Password reset",req}); res.json({ok:true}); });
adminRouter.patch("/users/:id/deactivate", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ if(req.admin?.id===req.params.id) throw new ApiError(403,"Ne možete deaktivirati svoj nalog.","FORBIDDEN"); const before=await prisma.adminUser.findUniqueOrThrow({where:{id:String(req.params.id)}}); if(req.admin?.role!=="OWNER" && before.role==="OWNER") throw new ApiError(403,"OWNER ne može biti deaktiviran.","FORBIDDEN"); const user=await prisma.adminUser.update({where:{id:String(req.params.id)},data:{isActive:false}}); await logAudit({adminId:req.admin?.id,action:"UPDATE",entity:"AdminUser",entityId:user.id,message:"Deactivate user",beforeJson:before,afterJson:user,req}); res.json({ok:true}); });
adminRouter.get("/apartments/:slug/rooms", requireAdmin, async(req,res)=>{ const apt=await prisma.apartment.findUniqueOrThrow({where:{slug:String(req.params.slug)}}); res.json({ok:true,rooms:await prisma.apartmentRoom.findMany({where:{apartmentId:apt.id},orderBy:{sortOrder:"asc"}})}); });
adminRouter.post("/apartments/:slug/rooms", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const apt=await prisma.apartment.findUniqueOrThrow({where:{slug:String(req.params.slug)}}); const body=z.object({name:z.string().min(1),area:z.string().optional().nullable(),floorMaterial:z.string().optional().nullable(),sortOrder:z.number().int().optional(),sourceStatus:z.enum(["VERIFIED","POTREBNA_PROVERA"]).default("POTREBNA_PROVERA"),sourceNote:z.string().optional().nullable()}).parse(req.body); const room=await prisma.apartmentRoom.create({data:{...body,apartmentId:apt.id}}); await logAudit({adminId:req.admin?.id,action:"CREATE",entity:"ApartmentRoom",entityId:room.id,afterJson:room,req}); res.status(201).json({ok:true,room}); });
adminRouter.patch("/rooms/:id", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const before=await prisma.apartmentRoom.findUniqueOrThrow({where:{id:String(req.params.id)}}); const body=z.object({name:z.string().min(1).optional(),area:z.string().optional().nullable(),floorMaterial:z.string().optional().nullable(),sortOrder:z.number().int().optional(),sourceStatus:z.enum(["VERIFIED","POTREBNA_PROVERA"]).optional(),sourceNote:z.string().optional().nullable()}).parse(req.body); const room=await prisma.apartmentRoom.update({where:{id:String(req.params.id)},data:body}); await logAudit({adminId:req.admin?.id,action:"UPDATE",entity:"ApartmentRoom",entityId:room.id,beforeJson:before,afterJson:room,req}); res.json({ok:true,room}); });
adminRouter.delete("/rooms/:id", requireAdmin, requireRole([...roles.manage]), async(req,res)=>{ const before=await prisma.apartmentRoom.findUniqueOrThrow({where:{id:String(req.params.id)}}); await prisma.apartmentRoom.delete({where:{id:String(req.params.id)}}); await logAudit({adminId:req.admin?.id,action:"DELETE",entity:"ApartmentRoom",entityId:String(req.params.id),beforeJson:before,req}); res.json({ok:true}); });

...
export async function validateImageFile(
...
): Promise<{ mime: AllowedImageMime; ext: "jpg" | "png" | "webp" | "avif" }> {
  if (file.size > MAX_IMAGE_BYTES)
    throw new ApiError(400, "Slika ne sme biti veća od 15 MB.", "IMAGE_TOO_LARGE");
  ...
  if (detected.mime !== file.mimetype)
    throw new ApiError(
      400,
      "Sadržaj fajla ne odgovara prijavljenom formatu.",
      "IMAGE_MIME_MISMATCH",
    );

...
export function ApartmentEditPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    adminFetch<{ apartment: Apartment }>(`/apartments/${slug}`)
      .then((data) => {
  ...
  async function save() {
    ...
    try {
      ...
      const data = await adminFetch<{ apartment: Apartment }>(`/apartments/${slug}`, {
        method: "PATCH",
  ...
  if (!apartment)
    return (
      <AdminShell>
        <p>Učitavanje apartmana...</p>
        {message ? <p className="mt-4 text-red-700">{message}</p> : null}
      </AdminShell>
    );
  ...
  return (
    <AdminShell>
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <Link className="text-sm text-forest-700" href="/admin/apartmani">
            ← Nazad na apartmane
          </Link>
          <h1 className="mt-2 text-3xl font-semibold">Uredi apartman {apartment.code}</h1>
        </div>
        <button
          disabled={saving}
          onClick={save}
          className="rounded-xl bg-forest-900 px-5 py-3 font-semibold text-white disabled:opacity-60"
        >
      ...
      <section className="grid gap-4 rounded-2xl bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        {[
          ["Šifra", apartment.code],
          ["Sprat", apartment.floor],
          ["Tip", apartment.officialType],
          ["Površina", `${apartment.marketArea} m²`],
          ["Source locked", apartment.sourceLocked ? "Da" : "Ne"],
        ].map(([label, value]) => (
          <label className="grid gap-2" key={label}>
            <span className="text-sm font-medium text-stone-600">{label}</span>
            <input disabled className="rounded-xl border bg-stone-100 p-3" value={value} />
          </label>
        ))}
      </section>
      <section className="mt-6 grid gap-4 rounded-2xl bg-white p-6 shadow-sm md:grid-cols-2">
        ...
        <label className="grid gap-2">
          ...
          <input
            ...
            placeholder="POTREBNA PROVERA"
          />
        </label>
        {["priceNote", "shortDescription", "seoTitle", "seoDescription"].map((key) => (
      ...
      <ApartmentMediaManager
        ...
        apartmentArea={apartment.marketArea}
      />
      <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-xl font-semibold">Advanced override za OWNER rolu</h2>
        <p className="mt-2 text-sm text-stone-600">
          Zvanična polja su zaključana. Izmene zahtevaju backend OWNER autorizaciju i obavezan audit
          razlog.
        </p>
        <label className="mt-4 flex items-center gap-3">
          <input
            type="checkbox"
            checked={Boolean(form.overrideOfficialFields)}
            onChange={(e) => setForm({ ...form, overrideOfficialFields: e.target.checked })}
          />{" "}
          overrideOfficialFields
        </label>
        <label className="mt-4 grid gap-2">
          <span>Audit razlog</span>
          <input
            className="rounded-xl border p-3"
            value={String(form.auditReason)}
            onChange={(e) => setForm({ ...form, auditReason: e.target.value })}
            placeholder="Obavezno za izmenu zvaničnih polja"
          />
        </label>
      </section>
    </AdminShell>

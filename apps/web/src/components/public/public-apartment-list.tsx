  ...
  return <>
    ...
    <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {merged.map((apartment) => <Link href={`/apartmani/${apartment.slug}`} className="group rounded-3xl border border-stone-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl" key={apartment.slug}>
        <div className="flex items-start justify-between"><div><p className="text-sm uppercase tracking-[0.3em] text-forest-700">{apartment.floor}</p><h3 className="mt-2 text-3xl font-semibold text-forest-950">{apartment.code}</h3></div><ArrowUpRight className="text-forest-700 transition group-hover:translate-x-1 group-hover:-translate-y-1" /></div>
        <div className="mt-8 grid grid-cols-3 gap-3 text-sm"><span><b>Tip</b><br />{apartment.officialType}</span><span><b>Površina</b><br />{apartment.marketArea.toFixed(2)} m²</span><span><b>Status</b><br />{statusLabels[apartment.status] ?? "POTREBNA PROVERA"}</span></div>
        {apartment.price ? <p className="mt-4 rounded-xl bg-forest-50 p-3 text-sm font-semibold">Cena: {apartment.price}</p> : <p className="mt-4 text-sm text-stone-500">Cena: POTREBNA PROVERA</p>}

...
export function PublicApartmentMedia({slug}:{slug:string}){const[m,setM]=useState<MediaAssetDto[]>([]);useEffect(()=>{publicFetch<{ok:true;media:MediaAssetDto[]}>(`/media?apartmentSlug=${slug}`).then(d=>setM(d?.media??[]))},[slug]); if(!m.length)return null; return <div className="mx-auto grid max-w-7xl gap-4 px-5 py-8 md:grid-cols-3">{m.map(x=><Image key={x.id} src={x.secureUrl} alt={x.alt??x.title} width={800} height={600} className="rounded-2xl"/> )}</div>}
...
function LiveMedia({placements}:{placements:string}){const[m,setM]=useState<MediaAssetDto[]>([]);useEffect(()=>{Promise.all(placements.split(',').map(p=>publicFetch<{ok:true;media:MediaAssetDto[]}>(`/media?placement=${p}`))).then(r=>setM(r.flatMap(x=>x?.media??[])))},[placements]); if(!m.length)return null; return <section className="mx-auto grid max-w-7xl gap-4 px-5 py-10 md:grid-cols-3">{m.map(x=>x.type==='VIDEO'||x.type==='VIRTUAL_TOUR'?<video key={x.id} src={x.secureUrl} controls className="rounded-2xl"/>:<Image key={x.id} src={x.secureUrl} alt={x.alt??x.title} width={800} height={600} className="rounded-2xl"/> )}</section>}

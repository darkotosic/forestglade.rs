      ...
      <div className="grid gap-5">
        <label className="grid gap-2 text-sm font-semibold text-forest-950">
          ...
          <input
            name="name"
            required
            className="rounded-2xl border border-stone-200 px-4 py-3 font-normal outline-none focus:border-gold-300"
          />
        </label>
        <label className="grid gap-2 text-sm font-semibold text-forest-950">
          Telefon
        ...
        <label className="grid gap-2 text-sm font-semibold text-forest-950">
          ...
          <select
            name="interestedIn"
            defaultValue="Kupovina apartmana"
            className="rounded-2xl border border-stone-200 px-4 py-3 font-normal outline-none focus:border-gold-300"
          >
            {options.map((option) => (
        ...
        <label className="flex gap-3 text-sm font-semibold text-forest-950">
          <input name="consentAccepted" type="checkbox" required className="mt-1" />
          Saglasan/saglasna sam da me Forest Glade kontaktira povodom upita.
        </label>
      </div>
      <button
        disabled={status === "loading"}
        className="mt-6 w-full rounded-full bg-forest-900 px-6 py-4 font-semibold text-white transition hover:bg-forest-700 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {status === "loading" ? "Slanje..." : "Pošaljite upit"}
      </button>
      {message ? (
        <p
          className={`mt-4 rounded-2xl px-4 py-3 text-sm ${status === "success" ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}
        >
          {message}
        </p>
      ) : null}

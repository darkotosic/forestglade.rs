    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-forest-950/90 shadow-2xl shadow-forest-950/10 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 text-sm font-medium text-mist-100 lg:flex">
          {navigation.map((item) => <Link key={item.href} href={item.href} className="rounded-full px-1 py-2 transition hover:text-gold-300">{item.label}</Link>)}
        </nav>
        <Link href="/kontakt" className="hidden rounded-full bg-gold-300 px-5 py-3 text-sm font-semibold text-forest-950 shadow-lg shadow-gold-300/15 transition hover:-translate-y-0.5 hover:bg-white sm:inline-flex">Zakažite prezentaciju</Link>
        <MobileNavigation />
      </div>
    </header>
  );
